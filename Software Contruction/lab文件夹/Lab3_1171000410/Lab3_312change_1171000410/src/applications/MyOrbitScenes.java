...
//		BufferedReader reader = null;
	...
	public void simulateAtomStrcture() {
		...
		String fileName = "txt/MyAtomicStructure.txt";
		...
		try {
			while (true) {
				...
				case "1":
					...
					break;
				case "2":
					System.out.println("请输入需要添加的轨道a,b,c");
					line = reader.readLine();
					items = line.split("\\s"); // 分开空白字符
					Track t1 = new Track(Double.parseDouble(items[0]),Double.parseDouble(items[1]),
		...
		try {
			while (true) {
				...
				case "3":
					...
					System.out.println("请输入行星<名称,形态,颜色,行星半径,轨道a,轨道b,轨道c,公转,速度,公转方向,初始位置的角度>");

					line = reader.readLine();
					items = line.split("\\s"); // 分开空白字符

					Planet planet = new Planet(items[0], items[1], items[2], translateData(items[3]),
							translateData(items[4]), translateData(items[5]),translateData(items[6]),
							translateData(items[7]),items[8], translateData(items[9]));
				...
				case "7":
					...
					for (int m = 0; m < T2E.size(); m++) {
						...
						for (Planet p : planetList) {
							cycle = 2 * Math.PI * p.getTrackA() / p.getSpeed(); // 秒
							...
							System.out.printf("%f天后%s的轨道a为%f,轨道b为%f,轨道c为%f,角度为%f\n", time, p.getName(), 
									p.getTrackA(),p.getTrackB(), p.getTrackC(),  sitha);
						}
				...
				case "8":
					...
					for (a = 0; a < T2E1.size(); a++) {
						...
						for (b = 0; b < pList.size(); b++) {
							if (pList.get(b).getName().equals(pName)) {  //用(a+b)/2代替r
								System.out.printf("%s与恒星的距离为%fkm\n", pName, (pList.get(b).getTrackA()+pList.get(b).getTrackB())/2);
							}
						}
					}
					break;
	...
	public void simulateSocialNetwork() {
		...
		String fileName = "txt/MySocialNetworkCircle.txt";
		File file = new File(fileName);
		soc.readFile(file);
