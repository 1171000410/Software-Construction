...
public class CircularOrbitAPIS<L, E> {
	...
	public double getPhysicalDistance(CircularOrbit c, E e1, E e2) {
		...
		double x1 = (p1.getTrackA() + p1.getTrackB())/2 * Math.sin(sitha1);
		double y1 = (p1.getTrackA() + p1.getTrackB())/2* Math.cos(sitha1);
		double x2 = (p2.getTrackA() + p2.getTrackB())/2 * Math.sin(sitha2);
		double y2 = (p2.getTrackA() + p2.getTrackB())/2 * Math.cos(sitha2);
			...
			if (tracksNumDifference >= 0) { // 前一个轨道数更多
				...
				for (i = 0; i < T2E1.size(); i++) {
					...
					for (int j = 0; j < T2E1.get(i).size(); j++) { // 找轨道2中没有的轨道1物体
						Planet p1 = (Planet) (T2E1.get(i).get(j));
						for (int k = 0; k < T2E2.get(i).size(); k++) {
							Planet p2 = (Planet) (T2E1.get(i).get(k));
							if (p1.getName() == p2.getName()) {
								flag1 = false;
					...
					for (int j = 0; j < T2E1.get(i).size(); j++) { // 找轨道1中没有的轨道2物体
						Planet p2 = (Planet) (T2E2.get(i).get(j));
						for (int k = 0; k < T2E1.get(i).size(); k++) {
							Planet p1 = (Planet) (T2E2.get(i).get(k));
							if (p1.getName() == p2.getName()) {
								flag2 = false;
								break; // 在轨道2找到该物体相等的物体，直接退出
							}
