...
public class MonkeyGenerator {
	...
	//   Generate monkeys ,GUI simulations
	//   and also record time, calculate throughput, calculate fairness
	
	public void produce(int n, int h, int t, int monkeyQuantity, int k, int mv) {
		...
		try {
			...
			for (int i = 0; i < times; i++) {
				for (int j = k * i; j < k * (i + 1); j++) {
					...
					m.setLadderStategy(randomStrategies[rs]);
			...
			if (monkeyQuantity % k != 0) {
				for (int q = k * times; q < monkeyQuantity; q++) {
					...
					int rd = random.nextInt(2);
					int rv = random.nextInt(mv) + 1;
					int rs = random.nextInt(3);   //产生随机数
					Monkey m = new Monkey(times * t, q + 1, randomDirections[rd], rv);
					m.setLadderStategy(randomStrategies[rs]);  //选取梯子
