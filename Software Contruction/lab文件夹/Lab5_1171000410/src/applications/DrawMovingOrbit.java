...
public class DrawMovingOrbit {
  ...
  private static ArrayList<ArrayList<Planet>> T2E = 
      new ArrayList<ArrayList<Planet>>(); // 物体所在轨道
  ...
  public DrawMovingOrbit(Circle center) {
    for (int i = 0; i < numOfTracks; i++) {
      ...
      for (int j = 0; j < T2E.get(i).size(); j++) {
        ...
        Circle newOrbiter = new Circle(new Point2D.Double(), 
            center.radius / 8, tick, orientation, i, colar);
  ...
  public static void visualizes(@SuppressWarnings("rawtypes") CircularOrbit c) {
    ...
    final DrawMovingOrbit d = new DrawMovingOrbit(new Circle(new 
        Point2D.Double(500, 500), 100, 1, 1, 1, ""));
  ...
  static class Circle {
    ...
    public void drawOrbit(Graphics2D g) {
      final int x = (int) (center.x - radius);
      final int y = (int) (center.y - radius);
